...
const DIG_COST=4, START_NUT=90, CORE_MAX=150;
...
const WAVE_INTERVAL=39000, FIRST_GRACE=36000, HERO_STAGGER=2200;
const VEIN_CAP=44;
...
const KINDS={
  slime:  {hp:10,  atk:2,  range:1, moveCd:560,  atkCd:720,  aggro:1, breedEvery:24000, breedCap:3, col:'#5fd16b'},
  carniv: {hp:26,  atk:5,  range:1, moveCd:590,  atkCd:680,  aggro:5, breedEvery:36000, breedCap:2, col:'#e06b3a'},
  evolved:{hp:90,  atk:16, range:1, moveCd:620,  atkCd:660,  aggro:5, breedEvery:0,     breedCap:1, col:'#9b2f4f'},   /* 上位種：直接増えず、卵で増える */
  spitter:{hp:16,  atk:6,  range:2, moveCd:590,  atkCd:980,  aggro:3, breedEvery:43000, breedCap:2, col:'#a64dff'},   /* 遠距離 */
  golem:  {hp:95,  atk:4,  range:1, moveCd:1100, atkCd:1050, aggro:4, breedEvery:0,     breedCap:1, col:'#6f86c4'},   /* 壁役：増えない */
  flame:  {hp:64,  atk:15, range:1, moveCd:590,  atkCd:780,  aggro:5, breedEvery:0,     breedCap:1, col:'#ff8a3a'},   /* 大火力：増えない */
  /* 上位種。eliteOf=元スプライト, tint=色替えフィルタ（形は元のまま、色だけ変える） */
  superslime:{hp:52, atk:7,  range:1, moveCd:520, atkCd:680, aggro:1, breedEvery:0, breedCap:2, col:'#7fbaff', eliteOf:'slime',   tint:'hue-rotate(150deg) saturate(1.35) brightness(1.05)', name:'スーパースライム'},
  tarantula: {hp:62, atk:15, range:2, moveCd:560, atkCd:880, aggro:4, breedEvery:0, breedCap:1, col:'#ff6b5a', eliteOf:'spitter', tint:'hue-rotate(120deg) saturate(1.4)',                 name:'タランチュラ'},
  titan:     {hp:220,atk:13, range:1, moveCd:1080,atkCd:1000,aggro:4, breedEvery:0, breedCap:1, col:'#d9b27a', eliteOf:'golem',   tint:'sepia(0.7) saturate(1.8) hue-rotate(-12deg) brightness(1.05)', name:'タイタン'},
  infernal:  {hp:150,atk:28, range:1, moveCd:560, atkCd:740, aggro:5, breedEvery:0, breedCap:1, col:'#5ab0ff', eliteOf:'flame',   tint:'hue-rotate(180deg) saturate(1.4) brightness(1.05)', name:'インフェルノ'},
};
...
function tryDig(col,row){
  ...
  if(tile.t==='bedrock'){ toast(col,row,'岩盤','#9b8fb0'); return; }
  ...
  if(!touch){ toast(col,row,'繋がってない','#d6543f'); return; }
  const cost=digCost(row);
  if(nutrients<cost){ toast(col,row,'栄養不足 '+cost,'#ffb84d'); return; }
  ...
  if(tile.sub){
    ...
    toast(col,row,KINDS[kind].name||VEIN[vein].legend.split('→')[1],'#ffe08a');
  } else {
    tile.t='tunnel';
  }
  tile.sub=null;
...
function killHero(h){
  ...
  const reward=Math.round(14+h.wave*4);
