...
const EVO_TIME=65000;         // 旧互換値。現在の鉱脈進化は時間ではなく魔物の接触回数で判定する。

/* 魔物（味方）の種別データ。breedEvery=下位種の直接増殖間隔(ms,0は増殖しない), breedCap=周囲(3x3)の同種上限 */
const KINDS={
  slime:  {hp:10,  atk:2,  range:1, moveCd:560,  atkCd:720,  aggro:1, rank:1, breedEvery:20000, breedCap:3, col:'#7fbaff'},
  ...
  superslime:{hp:52, atk:7,  range:1, moveCd:520, atkCd:680, aggro:1, rank:2, breedEvery:0, breedCap:2, col:'#e84a4a', eliteOf:'slime',   tint:'sepia(1) saturate(3) hue-rotate(-35deg) brightness(1.05)', name:'スーパースライム'},
  evolved:   {hp:90, atk:16, range:1, moveCd:620, atkCd:660, aggro:5, rank:6, breedEvery:0, breedCap:1, col:'#9b2f4f', eliteOf:'carniv',  tint:'hue-rotate(-35deg) saturate(1.55) brightness(0.95)', name:'進化肉食魔物'},
...
/* 鉱脈。魔物が隣接して触れるほど上位種鉱脈になる。後半種ほど必要接触数が多い。 */
const VEIN={
  moss:  {kind:'slime',   evoKind:'superslime', unlock:1, color:'#6fcf6f', core:'#bdf7bd', legend:'苔脈→スライム',    evoName:'上位苔脈', evoChance:0.22, touchNeed:4},
  meat:  {kind:'carniv',  evoKind:'evolved',    unlock:1, color:'#e63a2c', core:'#ffb39e', legend:'肉脈→肉食魔物',    evoName:'上位肉脈', evoChance:0.18, touchNeed:7},
  venom: {kind:'spitter', evoKind:'tarantula',  unlock:3, color:'#a64dff', core:'#e0bcff', legend:'毒脈→毒吐き',      evoName:'上位毒脈', evoChance:0.14, touchNeed:10, unlockMsg:'新たな鉱脈『毒脈』 ─ 毒吐きが眠る'},
  stone: {kind:'golem',   evoKind:'titan',      unlock:6, color:'#6f86c4', core:'#bcd0ff', legend:'石脈→岩兵',        evoName:'上位石脈', evoChance:0.12, touchNeed:13, unlockMsg:'新たな鉱脈『石脈』 ─ 岩兵が眠る'},
  ember: {kind:'flame',   evoKind:'infernal',   unlock:9, color:'#ffae26', core:'#ffe39a', legend:'火脈→炎魔',        evoName:'上位火脈', evoChance:0.10, touchNeed:16, unlockMsg:'新たな鉱脈『火脈』 ─ 炎魔が眠る'},
};
...
const PIXEL_ASSET_VERSION='v6-tdd-elite-palette-1';
...
function seedType(type, n, rMin, rMax){
  ...
  while(n>0 && tries<400 && veinCount()<VEIN_CAP){
    ...
    t.sub=type; t.age=0; t.evo=false; t.evoChecked=false; t.evoTouch=0; t.evoTouching={}; n--;
  }
}
