...
- M1: Godot CLI 起動、空のメインシーン、Web export preset、GUT実行を通す。
- M2: 11x16、48pxセル、土/通路/岩盤/入口/コア/鉱脈の盤面描画を再現する。
- M3: `tryDig`、鉱脈採掘、魔物出現、HUD更新、タップ/クリック入力を移植する。
- M4: 魔物AI、下位種増殖、上位種卵生成、卵孵化、捕食、視線判定、戦闘を移植する。
- M5: 勇者AI、経路探索、壁掘り、コア攻撃、ウェーブ、敗北/再開UIを移植する。
- M6: 既存仕様テストをGUTへ移植し、HTML/JSテストを退役する。
...
- 盤面初期化、採掘コスト、鉱脈採掘、上位鉱脈化。
- 下位種増殖、上位種卵生成、卵孵化、魔物/卵上限。
- 捕食、遠距離LOS、斜め壁角の射線遮断。
- 勇者クラス解禁、壁掘り、コア攻撃、ウェーブ人数上限。
- 長時間シミュレーションで例外が出ないこと。
